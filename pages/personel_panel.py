...
def render_menu_cards() -> None:
    """Ana sayfadaki 4 ana işlem kartını (Duyuru, Yemek, Arıza, Öğrenci) açıklamalı ve düzenli oluşturur."""
    
    # Kartların devasa görünmemesi için özel CSS
    st.markdown("""
        <style>
        .info-card {
    ...
    with row1_col1:
        st.markdown('<div class="info-card"><h3>📢 Duyuru Yönetimi</h3><p>Öğrencilere iletilecek genel duyuruları oluşturun ve sistemde yayınlayın.</p></div>', unsafe_allow_html=True)
        if st.button("Duyuruları Düzenle", use_container_width=True):
            st.session_state[SESSION_ADMIN_SUB_PAGE] = "duyuru"
            st.rerun()

    with row1_col2:
        st.markdown('<div class="info-card"><h3>🍴 Yemek Menüsü</h3><p>Günlük yemek listesini güncelleyerek öğrenci paneline yansıtın.</p></div>', unsafe_allow_html=True)
        if st.button("Menüyü Güncelle", use_container_width=True):
            st.session_state[SESSION_ADMIN_SUB_PAGE] = "yemek"
            st.rerun()

    with row2_col1:
        st.markdown('<div class="info-card"><h3>🛠️ Arıza Takibi</h3><p>Öğrencilerden gelen teknik arıza bildirimlerini görüntüleyin ve yönetin.</p></div>', unsafe_allow_html=True)
        if st.button("Arızaları Görüntüle", use_container_width=True):
            st.session_state[SESSION_ADMIN_SUB_PAGE] = "ariza"
            st.rerun()

    with row2_col2:
        st.markdown('<div class="info-card"><h3>👤 Öğrenci Kaydı</h3><p>Sisteme yeni katılacak öğrenciler için kullanıcı hesabı ve oda tanımlayın.</p></div>', unsafe_allow_html=True)
...
def render_stats(pending_count: int, solved_count: int, total_count: int) -> None:
    ...
    with col1:
        st.markdown(f'<div style="{box_style}"><div style="font-size: 24px; margin-bottom: 5px;">⏳</div><div style="font-size: 28px; font-weight: 800; color: #ef4444;">{pending_count}</div><div style="color: #64748b; font-size: 15px; font-weight: 600;">Beklemede</div></div>', unsafe_allow_html=True)
    with col2:
        st.markdown(f'<div style="{box_style}"><div style="font-size: 24px; margin-bottom: 5px;">✅</div><div style="font-size: 28px; font-weight: 800; color: #22c55e;">{solved_count}</div><div style="color: #64748b; font-size: 15px; font-weight: 600;">Çözüldü</div></div>', unsafe_allow_html=True)
    with col3:
        st.markdown(f'<div style="{box_style}"><div style="font-size: 24px; margin-bottom: 5px;">📋</div><div style="font-size: 28px; font-weight: 800; color: #0f172a;">{total_count}</div><div style="color: #64748b; font-size: 15px; font-weight: 600;">Toplam</div></div>', unsafe_allow_html=True)
    ...
    if st.button("Sistemde Yayınla", type="primary", use_container_width=True):
        if not title.strip() or not content.strip():
            ...
            if res.get("status") == "success":
                st.success("Duyuru tüm öğrencilere başarıyla iletildi!"); st.balloons()
    ...
    for fault in faults:
        ...
        with st.container(border=True):
            ...
            if c1.button("⏳ Beklemede", key=f"p_{fault_id}", use_container_width=True):
                if update_fault_api(fault_id, "Beklemede").get("status") == "success": st.rerun()
            if c2.button("✅ Çözüldü", key=f"s_{fault_id}", use_container_width=True):
                if update_fault_api(fault_id, "Çözüldü").get("status") == "success": st.rerun()
            if c3.button("❌ İptal Et", key=f"c_{fault_id}", use_container_width=True):
                if update_fault_api(fault_id, "İptal Edildi").get("status") == "success": st.rerun()
            if c4.button("🗑️ Sil", key=f"d_{fault_id}", type="primary", use_container_width=True):
                ...
                if delete_fault_api(fault_id).get("status") == "success": st.rerun()
